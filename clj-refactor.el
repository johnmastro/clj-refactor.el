...
(defun cljr--next-sexp (&optional arg)
  (interactive "p")
  (let ((n (or arg 1)))
    (paredit-forward (1+ n))
    (paredit-backward)))

(defun cljr--point-in-sexp? (pos)
  (let ((point (point))
        (end (save-excursion
               (paredit-forward)
               (point))))
...
(defun cljr--pairs-goal-col (pos)
  (goto-char (1+ pos))
  (let ((cols nil))
    (while (cljr--point-in-sexp? pos)
      (cljr--next-sexp)
...
;;;###autoload
(defun cljr-expand-pairs (&optional pos)
  (interactive)
  (let* ((pos        (or pos (cljr--find-start-of-pairs)))
         (last-left  (progn (goto-char (1+ pos))
                            (line-number-at-pos)))
         (last-right (progn (cljr--next-sexp)
                            (line-number-at-pos))))
    (cljr--next-sexp)
    (while (cljr--point-in-sexp? pos)
      (let ((line (line-number-at-pos)))
        (when (or (= line last-left)
                  (= line last-right))
          (paredit-newline)))
      (setq last-left (line-number-at-pos)
            last-right (save-excursion
                         (paredit-forward 2)
                         (line-number-at-pos)))
      (cljr--next-sexp 2))
    (goto-char pos)
...
;;;###autoload
(defun cljr-align-pairs ()
  (interactive)
  (let* ((pos  (cljr--find-start-of-pairs))
         (goal (progn (cljr-expand-pairs pos)
                      (cljr--pairs-goal-col pos))))
    (goto-char (1+ pos))
    (while (cljr--point-in-sexp? pos)
      (cljr--next-sexp)
